    ...
    return (<Container>
        <Row>
            <Col><h2>{currentClass.className}</h2>
                ...
                <Row><b>Videos</b><ListGroup as="ol">{currentClass.videos?.map(a => { const vid = new Video(a.videoId, a.title,a.description, a.videoSrc, a.seconds ); return (<ListGroup.Item as="li" key={a.videoId}><Col><Link href={`/class/${currentClass.classId}/video/${a.videoId}`}>{a.title}</Link></Col><Col>{vid.timeDisplay()}</Col></ListGroup.Item>)})}
                </ListGroup></Row>
            </Col>
...
export const getStaticProps: GetStaticProps = async (context) => {
    ...
    const creator : ContentCreator = await crResponse.json();
    return {
        props: {
